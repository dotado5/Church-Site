...
const createArticle = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (document !== null) {
            res.status(201).send(`Document successfully uploaded ${article.insertedId}`);
        }
...
const getAllArticles = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const articlesCollections = await database.collection('Articles').find({}).toArray();
...
const getArticleById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const document = await articlesCollections.findOne({ _id: new ObjectId(articleId) });
...
const getArticlesByAuthorId = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const authorArticles = articlesCollections.filter((article: any) => article.authorId === authorId);
...
async function deleteArticleById(req: Request, res: Response) {
    ...
    try {
        ...
        const result = await authorsCollection.deleteOne({ _id: new ObjectId(articleId) });
...
export { createArticle, getAllArticles, hello, getArticleById, getArticlesByAuthorId, deleteArticleById }

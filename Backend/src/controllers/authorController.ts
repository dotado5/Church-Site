...
const createAuthor = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (document !== null) {
            res.status(201).send(`Document successfully uploaded ${author.insertedId}`);
        }
...
const getAllAuthors = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const authorsCollection = await database.collection('Authors').find({}).toArray();
...
const getAuthorById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const document = await authorsCollection.findOne({ _id: new ObjectId(authorId) });
...
async function deleteAuthorById(req: Request, res: Response) {
    ...
    try {
        ...
        const result = await authorsCollection.deleteOne({ _id: new ObjectId(authorId) });
